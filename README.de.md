...
Die Oberfläche spricht **Deutsch und Englisch**, umschaltbar in der Kopfzeile.
Die Sprache hängt am Konto — ein deutsches und ein englisches Konto können also
nebeneinander arbeiten. Bis jemand angemeldet ist, richtet sich die
Anmeldemaske nach dem Browser.

Deutsch ist die Quelle: es steht im Markup und im Code so, wie es angezeigt
wird. Englisch liegt daneben in [`proxfy/static/i18n-en.js`](proxfy/static/i18n-en.js)
und, für Servermeldungen, in [`proxfy/texte.py`](proxfy/texte.py) — der
Schlüssel ist jeweils der deutsche Text selbst. Ein neues Feature heißt damit:
deutschen Text schreiben, eine englische Zeile ergänzen. Fehlt sie, erscheint
Deutsch — sichtbar und in einer Zeile behebbar, statt einer leeren Stelle oder
eines Schlüssels.
...
2. Unter *Einstellungen → Zugriff und Netzwerk* die Adresse von außen eintragen.
3. „Sitzungscookie nur über HTTPS" einschalten.
4. „Hinter einem Reverse Proxy betreiben" einschalten, damit die Sperre die echte
   Herkunftsadresse sieht. **Nur**, wenn wirklich ein Proxy davorsteht.
